	...
	if (check_input($_POST, $input_data) == false) {
		...
		if (check_empty($validation) == true) {
			...
			if ($method['count'] == 0) {
				...
				} elseif ($deposit_request['count'] >= 3) {
					$_SESSION['result'] = array('alert' => 'danger', 'title' => 'Gagal!', 'msg' => 'Anda tidak dapat membuat permintaan deposit lagi, karena Anda memiliki 3 permintaan berstatus <i>Pending</i>.');
				} else {
...
$bank = $method['rows']['name']; // BNI/BCA/GOPAY/OVO/INDOSAT/TELKOMSEL
...
<form class="form-horizontal" method="post" id="ajax-result">
	<input type="hidden" name="csrf_token" value="<?php echo $config['csrf_token'];	print_r($b); ?>">
	...
	<div class="form-group">
		<label>Jenis Permintaan</label><br />
			<label><input type="radio" name="type" value="auto" required checked> Otomatis</label><br />
			<!--<label><input type="radio" name="type" value="manual" required > Manual</label>-->
	</div>
	<div class="form-group">
...
<ul>
	...
	<li>Pilih jenis permintaan yang Anda inginkan, tersedia 2 opsi:
		<ul>
			...
			<li><b>Manual:</b> Anda harus melakukan konfirmasi pembayaran pada Admin <a href="http://bit.ly/konfirmasi_deposit_">( klik disini )</a>, agar permintaan deposit Anda dapat diterima.</li>
		</ul>
	</li>
