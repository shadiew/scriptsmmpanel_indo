...
if (mysqli_num_rows($check_order) == 0) {
  ...
  if ($o_provider == "MANUAL") {
    ...
    $check_provider = mysqli_query($db, "SELECT * FROM provider WHERE name = 'SMEDIA'");
    ...
    //print_r($chresult);
        ...
        if ($json_result['data']['status'] == "5") {
		    $status = "Success";
	    } elseif ($json_result['data']['status'] == "4") {
			$status = "Error";
		} elseif ($json_result['data']['status'] == "3") {
			$status = "Partial";
		} elseif ($json_result['data']['status'] == "2") {
			$status = "Processing";
		} else {
			$status = "Pending";
		}
